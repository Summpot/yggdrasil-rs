...
async fn test_lookup_cache_functionality() {
    ...
    // Add reachable node
    lookup_manager.add_reachable_node(peer_key, target_key).await;
    ...
    println!("First lookup: {:?}, Cached lookup: {:?}", first_lookup_time, cached_lookup_time);
...
async fn test_bloom_filter_false_positives() {
    ...
    println!("False positives: {}, True negatives: {}", false_positives, true_negatives);
    ...
    assert!(fp_rate < 0.1, "False positive rate too high: {:.2}%", fp_rate * 100.0);
}
