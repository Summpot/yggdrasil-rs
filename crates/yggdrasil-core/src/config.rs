...
pub struct Config {
    ...
    #[serde(rename = "PrivateKeyPath", skip_serializing_if = "Option::is_none", default)]
    ...
    #[serde(rename = "InterfacePeers", default, skip_serializing_if = "std::collections::HashMap::is_empty")]
    ...
    #[serde(rename = "AdminListen", default, skip_serializing_if = "Option::is_none")]
    ...
    #[serde(rename = "AllowedPublicKeys", default, skip_serializing_if = "Vec::is_empty")]
...
impl Config {
    ...
    pub fn to_hjson_with_comments(&self) -> Result<String> {
        ...
        output.push_str("  # socks://a.b.c.d:e/f.g.h.i:j). Connection strings can contain options.\n");
        output.push_str("  # Yggdrasil has no concept of bootstrap nodes - all network traffic\n");
        output.push_str("  # will transit peer connections. Therefore make sure to only peer with\n");
        ...
        // InterfacePeers
        output.push_str("  # List of connection strings for outbound peer connections in URI format,\n");
        output.push_str("  # arranged by source interface, e.g. { \"eth0\": [ \"tls://a.b.c.d:e\" ] }.\n");
        output.push_str("  # You should only use this option if your machine is multi-homed and you\n");
        output.push_str("  # want to establish outbound peer connections on different interfaces.\n");
        ...
        output.push_str("  # listeners in order to accept incoming peerings from non-local nodes.\n");
        output.push_str("  # Multicast peer discovery will work regardless of any listeners set\n");
        output.push_str("  # here. Each listener should be specified in URI format as above, e.g.\n");
        ...
        // AdminListen
        output.push_str("  # Listen address for admin connections. Default is to listen for local\n");
        ...
        // MulticastInterfaces
        output.push_str("  # Configuration for which interfaces multicast peer discovery should be\n");
        output.push_str("  # enabled on. Regex is a regular expression which is matched against an\n");
        output.push_str("  # interface name, and interfaces use the first configuration that they\n");
        output.push_str("  # match against. Beacon controls whether or not your node advertises its\n");
        output.push_str("  # presence to others, whereas Listen controls whether or not your node\n");
        ...
        // IfName
        output.push_str("  # Local network interface name for TUN adapter, or \"auto\" to select\n");
        ...
        output.push_str("  # Default is the largest supported size for your platform. The lowest\n");
        output.push_str("  # possible value is 1280.\n");
        output.push_str(&format!("  IfMTU: {}\n\n", self.if_mtu));

        // NodeInfoPrivacy
        output.push_str("  # By default, nodeinfo contains some defaults including the platform,\n");
    ...
    pub fn get_signing_key(&self) -> Result<SigningKey> {
        let private_key_bytes = self.private_key
    ...
    pub fn load_pem_private_key(&mut self, path: &str) -> Result<()> {
        ...
        if pem.tag() != "PRIVATE KEY" {
            return Err(anyhow::anyhow!("Expected PEM tag 'PRIVATE KEY', got '{}'", pem.tag()));
        }
        ...
        let seed = if contents.len() == 32 {
            ...
            return Err(anyhow::anyhow!("Invalid private key length: {}", contents.len()));
        };
    ...
    pub fn save_pem_private_key(&self, path: &str) -> Result<()> {
        ...
        fs::write(path, pem_str)
            .context(format!("Failed to write private key file: {}", path))?;
    ...
    pub fn get_certificate_pem(&self) -> Result<String> {
        let cert = self.certificate.as_ref()
            .ok_or_else(|| anyhow::anyhow!("No certificate available. Call generate_self_signed_certificate() first."))?;
...
mod tests {
    ...
    fn test_go_config_compatibility() {
        ...
        assert_eq!(config.admin_listen, Some("tcp://localhost:9001".to_string()));
        assert_eq!(config.if_name, "auto");
        assert_eq!(config.if_mtu, 65535);
    ...
    fn test_private_key_format_compatibility() {
        ...
        assert_eq!(addr32, addr64, "Both key formats should derive same address");
    ...
    fn test_config_round_trip() {
        ...
        assert_eq!(original_addr, parsed_addr, "Round-trip should preserve address derivation");
