...
use crate::link::{LinkManager, LinkEvent};
use crate::router::{RoutingTable, RouterEvent};
use crate::peer::{PeerManager, PeerInfo, ConnectionType, ConnectionState};
use crate::session::{SessionManager, Session};
use crate::spanning_tree::SpanningTree;
use crate::lookup::{LookupManager, BloomFilter};
use crate::tun_adapter::{TunAdapter, TunEvent};
use crate::proto::ProtoHandler;
use crate::admin::{
    AdminServer, GetSelfResponse, GetPeersResponse, PeerEntry, GetPathsResponse, PathEntry,
    GetSessionsResponse, SessionEntry, AddPeerResponse, RemovePeerResponse,
};
use anyhow::Result;
...
impl Core {
    ...
    pub async fn new(config: Config) -> Result<Self> {
        ...
        let (tun_adapter, tun_packet_rx, tun_event_rx) = if config.if_name != "none" {
            let (adapter, packet_rx, event_rx) = TunAdapter::new(
                config.if_name.clone(),
                config.if_mtu as u32,
            );
    ...
    async fn handle_link_event(&mut self, event: LinkEvent) -> Result<()> {
        match event {
            ...
            LinkEvent::HandshakeComplete(addr, peer_key, priority, is_inbound) => {
                info!("Handshake complete with {} (priority: {}, {})", 
                      addr, priority, if is_inbound { "inbound" } else { "outbound" });
                ...
                let mut peer = PeerInfo::new(
                    peer_key,
                    addr,
                    if is_inbound { ConnectionType::Incoming } else { ConnectionType::Outgoing },
                );
                ...
                if let Err(e) = self.spanning_tree.add_peer(peer_key).await {
                    ...
                    debug!("Added peer {} to spanning tree", hex::encode(&peer_key.to_bytes()[..8]));
                }
                ...
                match Session::derive_shared_secret(&private_key_bytes, &peer_key) {
                    Ok(shared_secret) => {
                        ...
                        if let Err(e) = self.session_manager.add_session(session).await {
                            ...
                            info!("Session established with {}", Address::from_public_key(&peer_key));
                        }
                    }
                    Err(e) => {
                ...
                self.lookup_manager.add_reachable_node(peer_key, peer_key).await;
                debug!("Added peer {} to lookup Bloom filter", hex::encode(&peer_key.to_bytes()[..8]));
                ...
                if let Err(e) = self.link_manager.send_to_peer(&addr, announcement_msg).await {
            ...
            LinkEvent::Disconnected(addr) => {
                ...
                if let Some(peer) = self.peer_manager.find_peer_by_addr(&addr).await {
                    ...
                    if let Err(e) = self.spanning_tree.remove_peer(&peer_key).await {
                        ...
                        debug!("Removed peer {} from spanning tree", hex::encode(&peer_key.to_bytes()[..8]));
                    }
            ...
            LinkEvent::DataReceived(addr, data) => {
                ...
                // Update statistics
                self.peer_manager.update_peer_stats(&addr, 0, data.len() as u64).await;
                
                // Find peer by address to get public key
                if let Some(peer) = self.peer_manager.find_peer_by_addr(&addr).await {
                    // Try to decrypt packet if session exists
                    let decrypted_data = if let Some(session) = self.session_manager.get_session(&peer.public_key).await {
                        match session.decrypt(&data) {
                            Ok(plaintext) => {
                                debug!("Decrypted {} bytes from peer", plaintext.len());
                                
                                // Update session statistics
                                self.session_manager.update_stats(&peer.public_key, 0, data.len() as u64).await;
                            ...
                            match decrypted_data[1] {
                                3 => {
                                    ...
                                    if decrypted_data.len() > 2 {
                                        if let Err(e) = self.handle_tree_announcement(&peer_key_bytes, &decrypted_data[2..]).await {
                                            warn!("Failed to handle tree announcement from {}: {}", addr, e);
                                        }
                                    }
                                }
                                4 => {
                                    ...
                                    if decrypted_data.len() > 2 {
                                        if let Err(e) = self.handle_bloom_filter_update(&peer_key_bytes, &decrypted_data[2..]).await {
                                            warn!("Failed to handle bloom filter from {}: {}", addr, e);
                                        }
                                    }
                                }
                                5 => {
                                    ...
                                    if decrypted_data.len() > 2 {
                                        if let Err(e) = self.handle_lookup_request(&peer_key_bytes, &decrypted_data[2..]).await {
                                            warn!("Failed to handle lookup request from {}: {}", addr, e);
                                        }
                                    }
                                }
                                6 => {
                                    ...
                                    if decrypted_data.len() > 2 {
                                        if let Err(e) = self.proto_handler.handle_proto(peer_key_bytes, &decrypted_data[1..]).await {
                                            warn!("Failed to handle lookup response from {}: {}", addr, e);
                                        }
                                    }
                                }
                                _ => {
                                    ...
                                    if let Err(e) = self.proto_handler.handle_proto(peer_key_bytes, &decrypted_data[1..]).await {
                                        warn!("Failed to handle protocol message from {}: {}", addr, e);
                                    }
                                }
                            }
                        }
                    ...
                    if decrypted_data.len() >= 40 {
                        ...
                        let _ = self.routing_table.update_route_from_packet(peer.public_key, &decrypted_data, latency).await;
                    }
                    
                    // Route decrypted packet
                    let local_addr = self.address.as_ipv6();
                    if let Err(e) = self.routing_table.handle_peer_packet(peer.public_key, decrypted_data, &local_addr).await {
    ...
    async fn handle_router_event(&mut self, event: RouterEvent) -> Result<()> {
        match event {
            RouterEvent::PacketToPeer(peer_key, data) => {
                debug!("Sending {} bytes to peer {}", data.len(), Address::from_public_key(&peer_key));
                ...
                if let Some(peer) = self.peer_manager.find_peer_by_key(&peer_key).await {
                    ...
                    let encrypted_data = if let Some(session) = self.session_manager.get_session(&peer_key).await {
                        match session.encrypt(&data) {
                            Ok(ciphertext) => {
                                debug!("Encrypted {} bytes for peer", ciphertext.len());
                                
                                // Update session statistics
                                self.session_manager.update_stats(&peer_key, ciphertext.len() as u64, 0).await;
                    ...
                    if let Err(e) = self.link_manager.send_to_peer(&peer.addr, encrypted_data.clone()).await {
                        warn!("Failed to send packet to peer {}: {}", peer.addr, e);
                    } else {
                        debug!("Sent {} bytes to peer at {}", encrypted_data.len(), peer.addr);
                    }
                    
                    // Update peer statistics
                    self.peer_manager.update_peer_stats(&peer.addr, encrypted_data.len() as u64, 0).await;
                } else {
                    warn!("No peer found with key {}", Address::from_public_key(&peer_key));
                }
            }
            ...
            RouterEvent::PacketFromPeer(peer_key, _data) => {
                debug!("Processing packet from peer {}", Address::from_public_key(&peer_key));
    ...
    pub async fn start(self: Arc<Self>) -> Result<()> {
        ...
        info!("Public key: {}", hex::encode(self.crypto.public_key().to_bytes()));
    ...
    async fn start_admin_server(self: &Arc<Self>) -> Result<()> {
        let admin_endpoint = self.config.admin_listen.as_ref()
                ...
                async move {
                    match request.as_str() {
                        ...
                        "addPeer" => {
                            ...
                            let response = core_clone.add_peer_handler(&req.uri, req.interface.as_deref()).await;
                            Ok(serde_json::to_value(response)?)
                        }
                        "removePeer" => {
                            let req: crate::admin::RemovePeerRequest = serde_json::from_value(args)?;
                            let response = core_clone.remove_peer_handler(&req.uri, req.interface.as_deref()).await;
                            Ok(serde_json::to_value(response)?)
                        }
                        _ => {
                            Err(anyhow::anyhow!("Unknown command: {}", request))
                        }
                    }
                }
            };
        ...
        let peer_entries = peers
            ...
            .map(|p| {
                ...
                PeerEntry {
                    ...
                    coords: if p.coords.is_empty() { None } else { Some(p.coords.clone()) },
    ...
    pub async fn get_sessions_info(&self) -> GetSessionsResponse {
        ...
        let session_entries = peers
            ...
            .map(|p| SessionEntry {
                ...
                latency_us: if p.rtt > 0 { Some(p.rtt as u64 * 1000) } else { None },
    ...
    async fn handle_tree_announcement(&self, peer_key_bytes: &[u8; 32], data: &[u8]) -> Result<()> {
        ...
        let coords = self.spanning_tree.calculate_coords(&peer_key).await;
        debug!("Updated coordinates for peer {}: {:?}", hex::encode(peer_key_bytes), coords);
    ...
    pub async fn broadcast_tree_announcement(&self) -> Result<()> {
        ...
        for peer in peers {
            // Encrypt if we have a session
            let data_to_send = if let Some(session) = self.session_manager.get_session(&peer.public_key).await {
                match session.encrypt(&packet) {
                    Ok(ciphertext) => ciphertext,
                    Err(e) => {
                        warn!("Failed to encrypt tree announcement for {}: {}", peer.addr, e);
            ...
            // Send via link manager
            if let Err(e) = self.link_manager.send_to_peer(&peer.addr, data_to_send).await {
                warn!("Failed to send tree announcement to {}: {}", peer.addr, e);
            }
        }
    ...
    pub async fn remove_peer_handler(&self, uri: &str, interface: Option<&str>) -> RemovePeerResponse {
    ...
    async fn handle_proto_send(&mut self, packet: Vec<u8>, to_key: [u8; 32]) -> Result<()> {
        ...
        if let Some(peer) = self.peer_manager.find_peer_by_key(&peer_key).await {
            ...
            // Send via link manager
            self.link_manager.send_to_peer(&peer.addr, encrypted_data).await?;
        } else {
            debug!("Cannot send protocol packet: peer {:?} not found", hex::encode(to_key));
        }
    ...
    async fn handle_bloom_filter_update(&self, peer_key: &[u8; 32], data: &[u8]) -> Result<()> {
        ...
        debug!("Received bloom filter update from {:?}", hex::encode(peer_key));
        ...
        self.lookup_manager.update_peer_filter(verifying_key, filter).await;
    ...
    async fn handle_lookup_request(&self, peer_key: &[u8; 32], data: &[u8]) -> Result<()> {
        ...
        debug!("Received lookup request from {:?} for target {:?}", 
               hex::encode(peer_key), hex::encode(target_key));

        // Check if we know the target
        let target_verifying_key = VerifyingKey::from_bytes(&target_key)?;
        let found_key = self.lookup_manager.lookup_node(&target_verifying_key).await?;

        // Send response
        let _peer_verifying_key = VerifyingKey::from_bytes(peer_key)?;
        if let Some(next_hop) = found_key {
            let response_key = next_hop.to_bytes();
            self.proto_handler.send_lookup_response(*peer_key, Some(response_key)).await?;
        } else {
            self.proto_handler.send_lookup_response(*peer_key, None).await?;
        }
