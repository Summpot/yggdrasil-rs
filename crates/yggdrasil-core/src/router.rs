...
impl RoutingTable {
    ...
    pub async fn add_route_with_coords(&self, entry: RouteEntry) -> Result<()> {
        ...
        if let Some(existing) = routes.get(&addr) {
            ...
            if is_better {
                debug!(
                    "Updating route to {} via better path (coords: {:?})",
                    addr, entry.coords
                );
    ...
    pub async fn route_packet_greedy(&self, dest_addr: &Ipv6Addr, packet: Vec<u8>) -> Result<()> {
        ...
        if let Some(route) = self.find_route(dest_addr).await {
            debug!(
                "Using direct route to {} via {} (hops: {})",
                dest_addr,
                Address::from_public_key(&route.next_hop),
                route.hops
            );
            ...
            if let Some(ref event_tx) = self.event_tx {
                event_tx
                    .send(RouterEvent::PacketToPeer(route.next_hop, packet))
                    .await
        ...
        let _spanning_tree = match self.spanning_tree.as_ref() {
            ...
            None => {
                warn!(
                    "No spanning tree available, cannot use greedy routing for {}",
                    dest_addr
                );
                self.send_icmp_unreachable(&packet, &Address::from(*dest_addr))
                    .await?;
                return Ok(());
            }
        };
        ...
        let routes = self.routes.read().await;
        let mut candidates: Vec<&RouteEntry> =
            routes.values().filter(|r| !r.coords.is_empty()).collect();

        if candidates.is_empty() {
            debug!(
                "No routes with coordinates available for greedy routing to {}",
                dest_addr
            );
            self.send_icmp_unreachable(&packet, &Address::from(*dest_addr))
                .await?;
        ...
        if let Some(ref event_tx) = self.event_tx {
            event_tx
                .send(RouterEvent::PacketToPeer(best.next_hop, packet))
                .await
                .map_err(|e| anyhow::anyhow!("Failed to send packet to peer: {}", e))?;
        }
    ...
    pub async fn route_packet(&self, packet: Vec<u8>) -> Result<()> {
        ...
        let dest_bytes: [u8; 16] = packet[24..40]
            .try_into()
        ...
        let src_bytes: [u8; 16] = packet[8..24]
            .try_into()
    ...
    async fn send_icmp_unreachable(&self, original_packet: &[u8], dest: &Address) -> Result<()> {
        ...
        if next_header == 58 {
            ...
            if original_packet.len() > 40 {
                ...
                if icmp_type >= 128 {
                    // Error messages are types 0-127
                    return Ok(());
                }
            }
        }
        ...
        debug!(
            "Sending ICMP destination unreachable to {} for {}",
            src_addr, dest
        );
    ...
    pub async fn handle_peer_packet(
        &self,
        ...
        if packet.len() < 40 {
            warn!(
                "Received packet too short to be valid IPv6: {} bytes",
                packet.len()
            );
            return Ok(());
        }
        ...
        if &dest_addr == local_addr {
            ...
            if let Some(ref event_tx) = self.event_tx {
                event_tx
                    .send(RouterEvent::PacketToTun(packet))
                    .await
    ...
    pub async fn update_route_from_packet(
        &self,
        peer_key: VerifyingKey,
        packet: &[u8],
        latency_ms: u32,
        ...
        if src_addr.segments()[0] == 0x0200 || src_addr.segments()[0] == 0x0300 {
            ...
            debug!(
                "Updated route to {} via peer {} (latency: {}ms)",
                src_addr,
                Address::from_public_key(&peer_key),
                latency_ms
            );
...
mod tests {
    ...
    async fn test_handle_peer_packet() {
        ...
        // Handle packet
        table
            .handle_peer_packet(peer_key, packet.clone(), &local_addr)
            .await
            .unwrap();
