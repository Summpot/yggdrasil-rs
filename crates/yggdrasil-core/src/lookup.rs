...
impl LookupManager {
    ...
    pub async fn lookup_node(&self, target_key: &VerifyingKey) -> Result<Option<VerifyingKey>> {
        ...
        if candidates.is_empty() {
            debug!(
                "No Bloom filter matches for target {}",
                hex::encode(&target_bytes[..8])
            );
            return Ok(None);
        }
    ...
    pub async fn start_lookup_request(
        &self,
        target_key: VerifyingKey,
    ) -> tokio::sync::oneshot::Receiver<VerifyingKey> {
        ...
        info!(
            "Started lookup request for {}",
            hex::encode(&target_key.to_bytes()[..8])
        );
    ...
    pub async fn handle_lookup_request(
        &self,
        target_key: VerifyingKey,
        from_peer: VerifyingKey,
    ) -> Result<()> {
        let target_bytes = target_key.to_bytes();
    ...
    pub async fn handle_lookup_response(
        &self,
        target_key: VerifyingKey,
        found_at: VerifyingKey,
    ) -> Result<()> {
        let target_bytes = target_key.to_bytes();
    ...
    pub async fn cleanup_expired_lookups(&self) {
        ...
        lookups.retain(|_, request| request.timestamp.elapsed() < LOOKUP_TIMEOUT);
