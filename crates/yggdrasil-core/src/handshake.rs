...
impl HandshakeMetadata {
    ...
    pub async fn decode<R: AsyncReadExt + Unpin>(reader: &mut R, password: &[u8]) -> Result<Self> {
        ...
        while pos + 4 <= fields.len() {
            ...
            match op {
                ...
                META_PUBLIC_KEY => {
                    let key_bytes: [u8; 32] = fields[pos..pos + 32]
                        .try_into()
    ...
    S: AsyncReadExt + AsyncWriteExt + Unpin,
{
    ...
    let peer_meta = tokio::time::timeout(timeout, recv_future)
        .await
    ...
    if let Some(allowed_keys) = allowed_public_keys {
        if !allowed_keys.is_empty() {
            ...
            if !allowed_keys
                .iter()
                .any(|k| k.to_lowercase() == peer_key_hex.to_lowercase())
            {
                anyhow::bail!("Public key not in whitelist: {}", peer_key_hex);
            }
        }
    }
...
mod tests {
    ...
    async fn test_metadata_round_trip() {
        ...
        let decoded = HandshakeMetadata::decode(&mut reader, password)
            .await
            .unwrap();
