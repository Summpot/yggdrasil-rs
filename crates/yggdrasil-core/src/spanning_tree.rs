...
impl TreeAnnouncement {
    ...
    pub fn decode(data: &[u8]) -> Result<Self> {
        ...
        let node_key = VerifyingKey::from_bytes(
            data[0..32]
                .try_into()
                .map_err(|_| anyhow::anyhow!("Invalid node key"))?,
        )
        ...
        let parent_key = VerifyingKey::from_bytes(
            data[32..64]
                .try_into()
                .map_err(|_| anyhow::anyhow!("Invalid parent key"))?,
        )
        ...
        let root_dist = u64::from_be_bytes(
            data[96..104]
                .try_into()
                .map_err(|_| anyhow::anyhow!("Invalid root_dist"))?,
        );
        ...
        Ok(Self::new(
            node_key, parent_key, root_key, root_dist, sequence,
        ))
    }
}
...
impl SpanningTree {
    ...
    pub fn new(local_key: VerifyingKey) -> Self {
        ...
        let initial_announcement = TreeAnnouncement::new(
            local_key, local_key, // We are our own parent
    ...
    pub async fn remove_peer(&self, peer_key: &VerifyingKey) -> Result<()> {
        ...
        if peers.remove(&peer_bytes).is_some() {
            info!(
                "Removing peer from spanning tree: {}",
                hex::encode(peer_bytes)
            );
    ...
    pub async fn handle_announcement(&self, announcement: TreeAnnouncement) -> Result<()> {
        ...
        if should_update {
            debug!(
                "Updated tree announcement for {}: root={}, dist={}",
    ...
    async fn update_parent(
        ...
        *seq += 1;
        ...
        let new_announcement =
            TreeAnnouncement::new(self.local_key, new_parent, new_root, new_dist, sequence);
    ...
    async fn select_new_parent(&self) -> Result<()> {
        ...
        if peers.is_empty() {
            ...
            self.update_parent(self.local_key, self.local_key, 0)
                .await?;
        }
...
mod tests {
    ...
    async fn test_tree_announcement_comparison() {
        ...
        assert!(
            ann2.is_newer_than(&ann1),
            "Announcement with smaller root key should win"
        );
    ...
    async fn test_parent_selection() {
        ...
        println!(
            "root < local: {}",
            root_key.as_bytes() < local_key.as_bytes()
        );
        ...
        let peer_announcement = TreeAnnouncement::new(
            peer_key, root_key, // peer's parent
