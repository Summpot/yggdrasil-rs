...
impl YggdrasilMetrics {
    ...
    pub async fn record_packet_sent(&self, bytes: u64) {
        self.registry.inc_counter("yggdrasil_packets_sent_total", 1).await;
        self.registry.inc_counter("yggdrasil_bytes_sent_total", bytes).await;
    }
    
    /// Record packet received
    pub async fn record_packet_received(&self, bytes: u64) {
        self.registry.inc_counter("yggdrasil_packets_received_total", 1).await;
        self.registry.inc_counter("yggdrasil_bytes_received_total", bytes).await;
    }
    
    /// Set peer count
    pub async fn set_peer_count(&self, count: i64) {
        self.registry.set_gauge("yggdrasil_peers_connected", count).await;
    }
    
    /// Set route count
    pub async fn set_route_count(&self, count: i64) {
        self.registry.set_gauge("yggdrasil_routing_entries", count).await;
    }
    
    /// Set session count
    pub async fn set_session_count(&self, count: i64) {
        self.registry.set_gauge("yggdrasil_sessions_active", count).await;
    }
    
    /// Record packet latency
    pub async fn record_latency(&self, latency_ms: f64) {
        self.registry.observe_histogram("yggdrasil_packet_latency_ms", latency_ms).await;
    }
    
    /// Record session establishment time
    pub async fn record_session_establishment(&self, duration_ms: f64) {
        self.registry.observe_histogram("yggdrasil_session_establishment_ms", duration_ms).await;
    }
    
    /// Set TUN device MTU
    pub async fn set_tun_mtu(&self, mtu: i64) {
        self.registry.set_gauge("yggdrasil_tun_mtu_bytes", mtu).await;
    }
    ...
    pub async fn set_tree_root_distance(&self, distance: i64) {
        self.registry.set_gauge("yggdrasil_tree_root_distance", distance).await;
    }
    
    /// Set lookup cache size
    pub async fn set_lookup_cache_size(&self, size: i64) {
        self.registry.set_gauge("yggdrasil_lookup_cache_entries", size).await;
    }
