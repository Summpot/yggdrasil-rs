...
impl TunAdapter {
    ...
    pub fn new(
        name: String,
        mtu: u32,
    ) -> (Self, mpsc::Receiver<Vec<u8>>, mpsc::Receiver<TunEvent>) {
        ...
        let device = {
            ...
            match tun::create(&config) {
                ...
                Err(e) => {
                    warn!(
                        "Failed to create TUN device (may need root privileges): {}",
                        e
                    );
...
mod tests {
    ...
    async fn test_tun_adapter_start() {
        ...
        let result = tokio::time::timeout(std::time::Duration::from_secs(5), adapter.start()).await;
